...
export async function getRelatedProducts(
  ...
  if (!isSanityConfigured) {
    return mockProducts
      .filter((p) => p.category?._id === categoryId && p._id !== productId)
      .slice(0, 4);
  }
  const data = await sanityClient.fetch<Product[]>(RELATED_PRODUCTS_QUERY, {
    categoryId,
    productId,
  });
  return data.length
    ? data
    : mockProducts.filter((p) => p.category?._id === categoryId && p._id !== productId).slice(0, 4);
}
